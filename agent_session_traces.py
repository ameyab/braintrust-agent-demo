#!/usr/bin/env python3
"""Agent variant that records each chat turn as a separate session-grouped trace."""
...
PROJECT = os.getenv("BRAINTRUST_PROJECT", "demo_ai_agent_session_traces")
...
def answer_question(
    ...
    """Run one independently traced agent turn."""
...
def chat() -> None:
    """Start a chat whose turns are separate traces in one Braintrust session."""
    ...
    print(f"Braintrust session: {session_id}")
    print("Each chat turn is recorded as a separate trace.")
    ...
    while True:
        ...
        # With no enclosing span, @traced makes chat_turn a new root trace.
        # answer_question logs this shared ID as metadata on every root span.
